...
if cat_end>0
    r_vector=linspace(cat_end,ParticleDiameter/2,NR);
else
    r_vector=linspace(0,ParticleDiameter/2,NR);
end
...
NS=length(MassFractionIN);

NEQ=NS+1;
y0=zeros((NR+1)*NEQ+1,1);
startSolidIndex=NEQ;
for i=1:NS
...
%% SYSTEM INTEGRATION
[t,y]=ode15s(@PFR_1D1D_heterogeneous,...
