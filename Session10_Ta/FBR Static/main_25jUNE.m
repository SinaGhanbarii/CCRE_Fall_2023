...
P0=101325;  % [Pa] 
T0=288+273.15;     % [K] temperature of the gas feed
rho_0=P0/8314/T0*MWmix;   % [kg/m3] density of the inlet gas feed
v_0=2.2e-2;     % [m/s] inlet superficial velocity

%% CATALYST PROPERTIES
catalyst_density=1270; %[kg/m3]
mcat=0.1; %[kgcat]
dp=108e-6; %[m]

%% GAS PROPERTIES
diffusivity=4.71e-5; % [m2/s]
viscosity=2.96e-5; % [Pa*s]
...
d_bubbles=9e-3; %[m] assumed constant for Geldart A particles
epsi_mf=0.405; % minimum fluidization void fraction
v_mf=3.67e-3;     % [m/s] minimum fluidization velocity
...
for i=1:NP
    for k=1:NS
        ...
        j=3;
            y_emulsion(i,k)=y(NTOT*(j-1)+NS*(i-1)+k);
    end
end
...
youtletNbut=(v_mf*y_emulsion(end,1)*V_emulsion+ v_bubble*y_bubble(end,1)*V_bubble)/(v_0*V_reactor);
youtletMA=(v_mf*y_emulsion(end,3)*V_emulsion+ v_bubble*y_bubble(end,3)*V_bubble)/(v_0*V_reactor);
