...
for i=1:NP
    for k=1:NS
        ...
        integral_function(i,k)= k_ce(i,k)*rho_interface*(y_cloud(i,k)-y_emulsion(k));
    end
end
...
for i=1:NP
    ...
    for k=1:NS
       ...
       molarfraction_emulsion(k)=y_emulsion(k)/MW(k);
    ...
    for k=1:NS
        ...
        xi_emulsion(k)=max(molarfraction_emulsion(k),0);
    end
    ...
    r1_emulsion=k1*(P0_atm*xi_emulsion(1))^0.54/(1+KMA*(P0_atm*xi_emulsion(3)));
    
    r2_bubble=k2*(P0_atm*xi_bubble(i,1))^0.54;
    r2_cloud=k2*(P0_atm*xi_cloud(i,1))^0.54;
    r2_emulsion=k2*(P0_atm*xi_emulsion(1))^0.54;
    ...
    r3_emulsion=k3*(P0_atm*xi_emulsion(3))/(1+KMA*(P0_atm*xi_emulsion(3)))^2;
    ...
    for k=1:NS
        ...
        Rate_emulsion(k)=rk_prodution_emulsion(k)*catalyst_density; %kmol/m3cat/s
    end
end
    ...
    for k=1:NS
        j=1; % bubble phase
            if i == 1
                ...
                +k_bc(i,k)*rho_interface*(y_cloud(i,k)-y_bubble(i,k)) ...
                +Rate_bubble(i,k)*MW(k)*gamma_b;
            end
        j=2; % cloud phase
            fluidized_bed(NTOT*(j-1)+NS*(i-1)+k)= -k_bc(i,k)*rho_interface_bc*(y_cloud(i,k)-y_bubble(i,k)) ...
                +k_ce(i,k)*rho_interface_ce*(y_emulsion(k)-y_cloud(i,k)) ...
                +Rate_cloud(i,k)*MW(k)*gamma_c;
       
        j=3; % emulsion phase
            fluidized_bed(NTOT*(j-1)+k)=(rho_0*v_mf)*(1-delta-(f_cl+f_w)*delta) ...
                /delta*(omega_0(k)-y_emulsion(k)) ...
                +Rate_emulsion(k)*MW(k)*gamma_e*L+integral(k); %integral equation
    end
end
end
